...
public class F3DBeam : MonoBehaviour
{
    ...
    void Raycast()
    {
        ...
        if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask) &&
            !hitPoint.collider.gameObject.CompareTag("PlayerBullet"))
        {
    ...
    private Vector2 tmp_V2 = Vector2.zero;
    void Update()
    {
        // Animate texture UV
        if (AnimateUV) 
        {
            tmp_V2.x = Time.time * UVTime + initialBeamOffset;
            lineRenderer.material.SetTextureOffset("_MainTex", tmp_V2);
        }
